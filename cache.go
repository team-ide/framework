...
func (this_ *Cache[T]) Remove(removeKeys ...string) {
	...
	this_.size = len(keys)
	if this_.outLog {
		Debug(fmt.Sprintf("删除 缓存 后 缓存大小 [%d]", this_.size))
	}
}
...
func (this_ *Cache[T]) cleanLimit() {
	...
	if removeKeySize > 0 {
		if this_.outLog {
			Debug(fmt.Sprintf("缓存数量 [%d] 超出最大数量 [%d]", keySize, this_.limit))
		}
...
func (this_ *Cache[T]) cleanup() {
	for {
		...
		case <-this_.ticker.C:
			...
			if this_.outLog {
				Debug(fmt.Sprintf("缓存过期的 key %s", removeKeys))
			}
