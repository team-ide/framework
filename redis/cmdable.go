...
func GetValueInfo(ctx context.Context, client redis.Cmdable, database int, key string, valueStart, valueSize int64) (valueInfo *ValueInfo, err error) {
	...
	if valueType == "none" {
		...
		if err != nil {
			err = errors.New("Get key [" + key + "] error:" + err.Error())
			return
		}
		...
		if err != nil {
			err = errors.New("LLen key [" + key + "] error:" + err.Error())
			return
		}
		...
		if err != nil {
			err = errors.New("LRange key [" + key + "] error:" + err.Error())
			return
		}
		...
		if err != nil {
			err = errors.New("SCard key [" + key + "] error:" + err.Error())
			return
		}
		valueInfo.ValueStart = valueStart
		valueInfo.ValueEnd = valueInfo.ValueStart + valueSize
		...
		if err != nil {
			err = errors.New("SScan key [" + key + "] error:" + err.Error())
			return
		}
		...
		if err != nil {
			err = errors.New("HLen key [" + key + "] error:" + err.Error())
			return
		}
		if valueSize <= 0 {
		...
		if err != nil {
			err = errors.New("HScan key [" + key + "] error:" + err.Error())
			return
		}
...
func (this_ *CmdService) Incr(key string, value int64, args ...Arg) (res int64, err error) {
	...
	cmd := client.IncrBy(param.Ctx, key, value)
	res, err = cmd.Result()
	if err != nil {
		return
	}
