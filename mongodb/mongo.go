...
func (this_ *Service) init() (err error) {
	...
	if this_.MinPoolSize > 0 {
		clientOptions.SetMinPoolSize(uint64(this_.MinPoolSize))
	}
	if this_.MaxPoolSize > 0 {
		clientOptions.SetMaxPoolSize(uint64(this_.MaxPoolSize))
	}
	if this_.ConnectTimeout > 0 {
		clientOptions.SetConnectTimeout(time.Millisecond * time.Duration(this_.ConnectTimeout))
	}
