...
func (*defaultLogger) Printf(format string, args ...interface{}) {
	fmt.Println(fmt.Sprintf("zookeeper log:"+format, args...))
}
...
func (this_ *Service) init(connProxy ConnProxy) (err error) {
	...
	if err != nil {
		err = errors.New("zk.Connect servers [" + strings.Join(this_.GetServers(), ",") + "] error: " + err.Error())
		if this_.zkConn != nil {
			this_.zkConn.Close()
		}
		return
	...
	if this_.Username != "" || this_.Password != "" {
		...
		if err != nil {
			err = errors.New("zk.Connect AddAuth servers [" + strings.Join(this_.GetServers(), ",") + "] error: " + err.Error())
			if this_.zkConn != nil {
				this_.zkConn.Close()
			}
			return
